...
/**
 * Get file format, image resolution, number of bits per pixel and optionally number of images, comments and physical resolution from JPEG, GIF, BMP,
 * PCX, PNG, IFF, RAS, PBM, PGM, PPM, PSD and SWF files (or input streams).
 * <p/>
 ...
 * </pre> You can also use this class as a command line program. Call it with a number of image file names and URLs as parameters:
 * <pre>
 *   java ImageInfo *.jpg *.png *.gif http://somesite.tld/image.jpg
 * </pre> or call it without parameters and pipe data to it:
 * <pre>
 ...
 * Known limitations:
 * <ul>
 * <li>When the determination of the number of images is turned off, GIF bits per pixel are only read from the global header. For some GIFs, local
 * palettes change this to a typically larger value. To be certain to get the correct color depth, call setDetermineImageNumber(true) before calling
 * check(). The complete scan over the GIF file will take additional time.</li>
 * <li>Transparency information is not included in the bits per pixel count. Actually, it was my decision not to include those bits, so it's a
 * feature! ;-)</li>
 * </ul>
 * <p/>
 * Requirements:
 ...
 * <li><strong>2002-01-24</strong> Added support for file formats Portable Bitmap / Graymap / Pixmap (PBM, PGM, PPM) and Adobe Photoshop (PSD). Added
 * new method getMimeType() to return the MIME type associated with a particular file format.</li>
 ...
 * <code>true</code> as argument to identify animated GIFs ({@link #getNumberOfImages()} will return a value larger than
 * <code>1</code>).</li>
 * <li><strong>2002-04-10</strong> Fixed a bug in the feature 'determine number of images in animated GIF' introduced with version 1.1. Thanks to
 * Marcelo P. Lima for sending in the bug report. Released as 1.1.1.</li>
 * <li><strong>2002-04-18</strong> Added {@link #setCollectComments(boolean)}. That new method lets the user specify whether textual comments are to
 * be stored in an internal list when encountered in an input image file / stream. Added two methods to return the physical width and height of the
 * image in dpi: {@link #getPhysicalWidthDpi()} and {@link #getPhysicalHeightDpi()}. If the physical resolution could not be retrieved, these methods
 * return
 * <code>-1</code>.
 * </li>
 ...
 * <li><strong>2002-06-17</strong> Added support for SWF, sent in by Michael Aird. Changed checkJpeg() so that other APP markers than APP0 will not
 * lead to a failure anymore. Released as 1.3.</li>
 * <li><strong>2003-07-28</strong> Bug fix - skip method now takes return values into consideration. Less bytes than necessary may have been skipped,
 * leading to flaws in the retrieved information in some cases. Thanks to Bernard Bernstein for pointing that out. Released as 1.4.</li>
 * <li><strong>2004-02-29</strong> Added support for recognizing progressive JPEG and interlaced PNG and GIF. A new method {@link #isProgressive()}
 * returns whether ImageInfo has found that the storage type is progressive (or interlaced). Thanks to Joe Germuska for suggesting the feature. Bug
 * fix: BMP physical resolution is now correctly determined. Released as 1.5.</li>
 * </ul>
 */
public class ImageInfo {

    public ImageInfo(String ruta) {
        imageInfo.setDetermineImageNumber(true);
        try {
            fichero = ruta;
            InputStream in2 = new FileInputStream(ruta);
            in = in2;
            run(ruta, in2, imageInfo, true);
    ...
    /**
     * Return value of {@link #getFormat()} for JPEG streams. ImageInfo can extract physical resolution and comments from JPEGs (only from APP0
     * headers). Only one image can be stored in a file. It is determined whether the JPEG stream is progressive (see {@link #isProgressive()}).
     */
    public static final int FORMAT_JPEG = 0;
    /**
     * Return value of {@link #getFormat()} for GIF streams. ImageInfo can extract comments from GIFs and count the number of images (GIFs with more
     * than one image are animations). If you know of a place where GIFs store the physical resolution of an image, please
     * <a href="http://www.geocities.com/marcoschmidt.geo/contact.html">send me a mail</a>! It is determined whether the GIF stream is interlaced (see
     * {@link #isProgressive()}).
     */
    public static final int FORMAT_GIF = 1;
    /**
     * Return value of {@link #getFormat()} for PNG streams. PNG only supports one image per file. Both physical resolution and comments can be stored
     * with PNG, but ImageInfo is currently not able to extract those. It is determined whether the PNG stream is interlaced (see
     * {@link #isProgressive()}).
     */
    public static final int FORMAT_PNG = 2;
    /**
     * Return value of {@link #getFormat()} for BMP streams. BMP only supports one image per file. BMP does not allow for comments. The physical
     * resolution can be stored.
     */
    public static final int FORMAT_BMP = 3;
    /**
     * Return value of {@link #getFormat()} for PCX streams. PCX does not allow for comments or more than one image per file. However, the physical
     * resolution can be stored.
     */
    ...
    /**
     * Return value of {@link #getFormat()} for RAS streams. Sun Raster allows for one image per file only and is not able to store physical
     ...
     */
    public static final String[] FORMAT_NAMES = {"jpeg", "gif", "png", "bmp", "pcx",
        "iff", "ras", "pbm", "pgm", "ppm",
        "psd", "swf", "tiff"};
    /**
    ...
    public static final String[] MIME_TYPE_STRINGS = {"image/jpeg", "image/gif", "image/png", "image/bmp", "image/pcx",
        ...
        "image/psd", "application/x-shockwave-flash", "image/tiff"};
    private int width;
    private int height;
    ...
    /**
     * Call this method after you have provided an input stream or file using {@link #setInput(java.io.InputStream)} or
     * {@link #setInput(java.io.DataInput)}. If true is returned, the file format was known and information on the file's content can be retrieved
    ...
    private boolean checkTiff() {
        ...
        try {
            ...
            for (int i = 0; i < allTiffFields.length; i++) {
                ...
                if (numberOfField == 282) {
                    xdpi = Integer.parseInt(valueOfField.substring(0, valueOfField.indexOf("/")));
                }
                if (numberOfField == 283) {
                    ydpi = Integer.parseInt(valueOfField.substring(0, valueOfField.indexOf("/")));
                }
            }
            width = dec.decodeAsRenderedImage().getWidth();
            height = dec.decodeAsRenderedImage().getHeight();
            setPhysicalHeightDpi(ydpi);
            setPhysicalWidthDpi(xdpi);
            bitsPerPixel = dec.decodeAsRenderedImage().getColorModel().getPixelSize();
            in.close();
        } catch (Exception ex) {
            System.out.println("Error al leer el fichero de imagen " + fichero + " - " + ex.getMessage());
        }
        return true;
    }
    ...
    /**
     * If {@link #check()} was successful, returns the image's number of bits per pixel. Does not include transparency information like the alpha
     * channel.
     *
     ...
     *
     * @throws java.lang.IllegalArgumentException if index is smaller than 0 or larger than or equal to the number of comments retrieved
    ...
    /**
     * If {@link #check()} was successful, returns the image format as one of the FORMAT_xyz constants from this class. Use {@link #getFormatName()}
     ...
     * <code>true</code> as argument, returns the number of comments retrieved from the input image stream / file. Any number &gt;= 0 and smaller than
     ...
     * <code>setDetermineImageNumber(true);</code> was called before a successful call to {@link #check()}. This value can currently be only different
     * from
    ...
    public float getPhysicalHeightCm() {
        return (float) (getPhysicalHeightInch() * 2.54);
    }
     ...
     * <code>-1.0f</code> if width information is not available. Assumes that {@link #check} has been called successfully.
     *
    ...
    public float getPhysicalWidthCm() {
        return (float) (getPhysicalWidthInch() * 2.54);
    }
    ...
    /**
     * To use this class as a command line application, give it either some file names as parameters (information on them will be printed to standard
     * output, one line per file) or call it with no parameters. It will then check data given to it via standard input.
     *
     * @param args the program arguments which must be file names
     */
    public static String fichero = "";
    static ImageInfo imageInfo = new ImageInfo();
    ...
    public static void main(String[] args) {
        ...
        InputStream in = null;
        try {
            String name = "C:\\Users\\julian.collado\\Documents\\juanqui-luis.tif";
            fichero = name;
    ...
    private static void printCompact(String sourceName, ImageInfo imageInfo) {
        System.out.println(imageInfo.getFormatName() + ";"
                ...
                + imageInfo.getPhysicalWidthCm() + ";"
                + imageInfo.getPhysicalHeightCm() + ";"
    ...
    private static void printVerbose(String sourceName, ImageInfo ii) {
        ...
        printLine(1, "Physical width (cm): ", decimalformat.format(ii.getPhysicalWidthCm()));
        printLine(1, "Physical height (cm): ", decimalformat.format(ii.getPhysicalHeightCm()));
...
//        }
     ...
     * <code>false</code>. This is a special option because some file formats require running over the entire file to find out the number of images, a
     * rather time-consuming task. Not all file formats support more than one image. If this method is called with
     * <code>true</code> as argument, the actual number of images can be queried via {@link #getNumberOfImages()} after a successful call to
     * {@link #check()}.
     *
